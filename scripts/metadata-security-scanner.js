...
async function convertXmlToJson(xmlFilePath, outputDir) {
  ...
  return new Promise((resolve, reject) => {
    parser.parseString(xmlContent, (err, result) => {
      ...
      const basename = path.basename(xmlFilePath).replace(/\..*-meta\.xml$/, '');
...
async function main() {
  ...
  if (!fs.existsSync('sfdx-project.json')) {
    console.error('❌ This is not a valid SFDX project (sfdx-project.json not found)');
    process.exit(1);
  }
  
  // Check if .policies directory exists
  if (!fs.existsSync('.policies')) {
    console.log('⚠️  No .policies directory found. Skipping security scan.');
    process.exit(0);
  }
  ...
  try {
    execSync(`sf sgd source delta --to "HEAD" --from "origin/${process.env.GITHUB_BASE_REF || 'main'}" --output-dir "${sgdOutput}" --generate-delta`, 
      { stdio: 'inherit' });
  } catch (error) {
  ...
  console.log('🔍 Scanning changed permission sets for security violations...\n');
  ...
  const findPermissionSets = (dir, files = []) => {
    ...
    for (const item of items) {
      ...
      } else if (item.isFile() && item.name.endsWith('.permissionset-meta.xml')) {
        files.push(itemPath);
      }
    }
  ...
  console.log(`Found ${permissionSetFiles.length} changed permission set(s)\n`);
  ...
  for (const filePath of permissionSetFiles) {
    ...
    try {
      ...
      const policyFiles = fs.readdirSync(policyPath)
      ...
      for (const policyFile of policyFiles) {
        ...
        try {
          ...
          if (violations && violations.deny && Object.keys(violations.deny).length > 0) {
          ...
          if (violations && violations.warn && Object.keys(violations.warn).length > 0) {
          ...
          console.log(`    ⚠️  Policy execution failed: ${opaError.message}`);
        }
      }
    } catch (conversionError) {
      console.log(`  ⚠️  Failed to convert ${filePath}: ${conversionError.message}`);
    }
  ...
  if (totalViolations === 0) {
    ...
    console.log('All permission set changes comply with security policies.');
    process.exit(0);
  } else {
